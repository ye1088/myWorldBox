...
final class PooledDirectByteBuf extends PooledByteBuf<ByteBuffer> {
    ...
    protected int _getUnsignedMedium(int index) {
        index = idx(index);
        return (((ByteBuffer) this.memory).get(index + 2) & 255) | (((((ByteBuffer) this.memory).get(index) & 255) << 16) | ((((ByteBuffer) this.memory).get(index + 1) & 255) << 8));
    }

    protected int _getUnsignedMediumLE(int index) {
        index = idx(index);
        return ((((ByteBuffer) this.memory).get(index + 2) & 255) << 16) | ((((ByteBuffer) this.memory).get(index) & 255) | ((((ByteBuffer) this.memory).get(index + 1) & 255) << 8));
    }
    ...
    private int getBytes(int index, FileChannel out, long position, int length, boolean internal) throws IOException {
        ...
        ByteBuffer tmpBuf = internal ? internalNioBuffer() : ((ByteBuffer) this.memory).duplicate();
        index = idx(index);
        tmpBuf.clear().position(index).limit(index + length);
