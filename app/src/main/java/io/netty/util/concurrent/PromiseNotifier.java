        ...
        if (future.isSuccess()) {
            ...
            for (Promise<? super V> p2 : this.promises) {
                if (!p2.cancel(false) && this.logNotifyFailure) {
                    logger.warn("Failed to cancel a promise because it is done already: {}", p2);
                }
            }
        } else {
            ...
            for (Promise<? super V> p22 : this.promises) {
                if (!p22.tryFailure(cause) && this.logNotifyFailure) {
                    logger.warn("Failed to mark a promise as failure because it's done already: {}", p22, cause);
                }
            }
        }
    }
