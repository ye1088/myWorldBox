...
public class EditAnnounceActivity extends HTBaseLoadingActivity {
    ...
    private void ER() {
        try {
            if (b.Em().contains(com.MCWorld.utils.t.bkR)) {
                c draft = (c) Json.parseJsonObject(b.Em().getString(com.MCWorld.utils.t.bkR), c.class);
                if (draft != null) {
    ...
    private void ES() {
        ...
        draft.aC(content);
        b.Em().putString(com.MCWorld.utils.t.bkR, Json.toJson(draft));
    }

    private void ET() {
        if (b.Em().contains(com.MCWorld.utils.t.bkR)) {
            b.Em().remove(com.MCWorld.utils.t.bkR);
        }
    }
