...
public class UtilsFunction {
    ...
    public static <E> Pred<E> negate(Pred<E> p) {
        return new 1(p);
    }
    ...
    public static <E> List<E> nubBy(Eq<E> cmp, List<E> xs) {
        List ys = new ArrayList();
        if (!empty((Collection) xs)) {
            for (E x : xs) {
                if (find(new 3(cmp, x), ys) == null) {
                    ys.add(x);
                }
            }
        }
    ...
    public static boolean notEmpty(Collection<?> xs) {
        return (xs == null || xs.isEmpty()) ? false : true;
    }
