...
public class e extends c implements a {
    ...
    public com.MCWorld.controller.resource.action.e nQ;
    ...
    public void aj(String path) {
        ...
        if (UtilsFile.availableSpace(desFile.getAbsolutePath()) < ((long) (((float) new File(path).length()) * 1.3f))) {
            ...
            EventNotifyCenter.notifyEvent(com.MCWorld.controller.c.class, 267, new Object[]{((ResTaskInfo) getInfo()).url});
            return;
        }
        this.nQ = new com.MCWorld.controller.resource.action.e(new File(path), desFile.getAbsolutePath(), this);
        this.nQ.run();
        ((ResTaskInfo) getInfo()).mR = new File(desFile, "app.apk").getAbsolutePath();
        ((ResTaskInfo) getInfo()).state = State.UNZIP_START.ordinal();
        EventNotifyCenter.notifyEventUiThread(com.MCWorld.controller.c.class, 260, new Object[]{((ResTaskInfo) getInfo()).url});
        b hpkFileList = com.MCWorld.controller.resource.zip.c.ea().aq(((ResTaskInfo) getInfo()).url);
        if (hpkFileList == null) {
            hpkFileList = new b();
        ...
        if (((ResTaskInfo) getInfo()).state == State.UNZIP_COMPLETE.ordinal()) {
            ...
            AppConfig.getInstance().getUiHandler().post(new Runnable(this) {
                ...
                public void run() {
                    ...
                    EventNotifyCenter.notifyEvent(com.MCWorld.controller.c.class, 265, new Object[]{((ResTaskInfo) this.oF.getInfo()).url, apk, signature});
                }
            });
        }
    }
    ...
    public void d(String srcFile, int progress, int length) {
        ...
        EventNotifyCenter.notifyEventUiThread(com.MCWorld.controller.c.class, 261, new Object[]{((ResTaskInfo) getInfo()).url});
    }
    ...
    public void r(String srcFile, int result) {
        ...
        EventNotifyCenter.notifyEvent(com.MCWorld.controller.c.class, 262, new Object[]{((ResTaskInfo) getInfo()).url});
        synchronized (this.LOCK) {
            this.LOCK.notifyAll();
        }
    }
