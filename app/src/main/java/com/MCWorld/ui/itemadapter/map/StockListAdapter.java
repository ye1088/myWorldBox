...
public class StockListAdapter extends BaseAdapter {
    ...
    public StockListAdapter(Activity context, List<Object> objects) {
        this.objs = objects;
        this.aTX = context;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        List<InventorySlot> cates = new ArrayList();
        for (int i = 0; i < this.DIV; i++) {
            int pos = (this.DIV * position) + i;
            if (pos < this.objs.size()) {
                cates.add((InventorySlot) this.objs.get(pos));
            }
        }
        StockItemAdapter adapter = new StockItemAdapter(this.aTX, cates, this.aTY);
