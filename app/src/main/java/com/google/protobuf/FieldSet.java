...
final class FieldSet<FieldDescriptorType extends FieldDescriptorLite<FieldDescriptorType>> {
    ...
    private void mergeFromField(Entry<FieldDescriptorType, Object> entry) {
        ...
        Object field;
        if (descriptor.isRepeated()) {
            field = getField(descriptor);
            if (field == null) {
                field = new ArrayList();
            }
            for (Object element : (List) value) {
                ((List) field).add(cloneIfMutable(element));
            }
            this.fields.put(descriptor, field);
        } else if (descriptor.getLiteJavaType() == JavaType.MESSAGE) {
            field = getField(descriptor);
            if (field == null) {
                this.fields.put(descriptor, cloneIfMutable(value));
                return;
            }
            this.fields.put(descriptor, descriptor.internalMergeFrom(((MessageLite) field).toBuilder(), (MessageLite) value).build());
        } else {
            this.fields.put(descriptor, cloneIfMutable(value));
        }
    }
