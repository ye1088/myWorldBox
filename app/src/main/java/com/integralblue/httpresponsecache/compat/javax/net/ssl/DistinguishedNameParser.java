...
public final class DistinguishedNameParser {
    ...
    private String nextAT() {
        while (this.pos < this.length && this.chars[this.pos] == HttpConstants.SP_CHAR) {
            this.pos++;
        }
        ...
        this.beg = this.pos;
        this.pos++;
        while (this.pos < this.length && this.chars[this.pos] != '=' && this.chars[this.pos] != HttpConstants.SP_CHAR) {
            this.pos++;
        }
        if (this.pos >= this.length) {
        ...
        this.end = this.pos;
        if (this.chars[this.pos] == HttpConstants.SP_CHAR) {
            while (this.pos < this.length && this.chars[this.pos] != '=' && this.chars[this.pos] == HttpConstants.SP_CHAR) {
                this.pos++;
            }
    ...
    private String quotedAV() {
        ...
        while (this.pos != this.length) {
            if (this.chars[this.pos] == StringUtil.DOUBLE_QUOTE) {
                this.pos++;
                while (this.pos < this.length && this.chars[this.pos] == HttpConstants.SP_CHAR) {
                    this.pos++;
                }
    ...
    private String hexAV() {
        ...
        this.beg = this.pos;
        this.pos++;
        while (this.pos != this.length && this.chars[this.pos] != '+' && this.chars[this.pos] != StringUtil.COMMA && this.chars[this.pos] != ';') {
            if (this.chars[this.pos] == HttpConstants.SP_CHAR) {
                this.end = this.pos;
                this.pos++;
                while (this.pos < this.length && this.chars[this.pos] == HttpConstants.SP_CHAR) {
        ...
        while (this.pos < this.length) {
            ...
            switch (this.chars[this.pos]) {
                case ' ':
                    ...
                    if (this.pos != this.length && this.chars[this.pos] != StringUtil.COMMA && this.chars[this.pos] != '+' && this.chars[this.pos] != ';') {
                        break;
                    }
    ...
    public String find(String attributeType) {
        ...
        do {
            ...
            if (this.chars[this.pos] == StringUtil.COMMA || this.chars[this.pos] == ';' || this.chars[this.pos] == '+') {
                this.pos++;
                attType = nextAT();
            } else {
                throw new IllegalStateException("Malformed DN: " + this.dn);
