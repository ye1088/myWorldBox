...
public class DiskBasedCache implements Cache {
    ...
    public DiskBasedCache(File rootDirectory, long maxCacheSizeInBytes) {
        ...
        HLog.info(TAG, "disk base cache init dir " + rootDirectory + ", size " + this.mMaxCacheSizeInBytes, new Object[0]);
    }
            ...
            if (entry2 != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        cis = cis2;
                        try {
                            HttpLog.e("%s: %s", file.getAbsolutePath(), e.toString());
                            remove(key);
                            if (cis != null) {
                                try {
                                    cis.close();
    ...
    public synchronized void initialize() {
        ...
        synchronized (this) {
            if (this.mRootDirectory.exists()) {
                ...
                HttpLog.e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
            }
        }
        return;
        if (fis != null) {
    ...
    public synchronized void put(String key, Entry entry) {
        pruneIfNeeded((long) entry.data.length);
        File file = getFileForKey(key);
        try {
    ...
    public synchronized void remove(String key) {
        ...
        removeEntry(key);
        if (!deleted) {
            HttpLog.d("Could not delete cache entry for key=%s, filename=%s", key, getFilenameForKey(key));
        }
    }
        ...
        if (this.mTotalSize + neededSpace >= this.mMaxCacheSizeInBytes) {
            ...
            while (iterator.hasNext()) {
                ...
                if (getFileForKey(e.key).delete()) {
                    ...
                    HttpLog.d("Could not delete cache entry for key=%s, filename=%s", e.key, getFilenameForKey(e.key));
                }
                iterator.remove();
                prunedFiles++;
                if (((float) (this.mTotalSize + neededSpace)) < ((float) this.mMaxCacheSizeInBytes) * HYSTERESIS_FACTOR) {
                    break;
                }
            }
            HttpLog.v("pruned %d files, %d bytes, %d ms", Integer.valueOf(prunedFiles), Long.valueOf(this.mTotalSize - before), Long.valueOf(SystemClock.elapsedRealtime() - startTime));
        }
    }
    ...
    private static byte[] streamToBytes(InputStream in, int length) throws IOException {
        if (length < 0) {
            throw new IOException("Expected " + length + " bytes, negative length");
        }
    ...
    static String readString(InputStream is) throws IOException {
        ...
        if (n <= TarBuffer.DEFAULT_BLKSIZE) {
            return new String(streamToBytes(is, n), "UTF-8");
        }
    ...
    static Map<String, String> readStringStringMap(InputStream is) throws IOException {
        int size = readInt(is);
        Map<String, String> result = (size <= 0 || size > 10000) ? Collections.emptyMap() : new HashMap(size);
