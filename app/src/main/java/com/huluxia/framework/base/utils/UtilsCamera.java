...
public class UtilsCamera {
    ...
    public static void fromCamera(Activity act) {
        if (UtilsFile.isSdcardMounted()) {
            ...
            if (UtilsApkPackage.availableIntent(act, intent)) {
                act.startActivityForResult(intent, 256);
                return;
            } else {
                Toast.makeText(act, "无法打开相机", 0).show();
                return;
            }
        }
        Toast.makeText(act, "没有安装SD卡，不能使用相机", 0).show();
    }
    ...
    public static void cropImage(Activity act, Class intentclass, Uri fromUri, Uri cropUri, int width, int height, boolean fixedAspectRatio) {
        String fromPath = UtilsFile.uriToPath(act, fromUri);
        String outputPath = cropUri.getPath();
        Intent cropIntent = new Intent();
        cropIntent.setClass(act.getApplicationContext(), intentclass);
    ...
    public static String onPickResultToCrop(int resultCode, int requestCode, Intent data, Activity act, Class intentclass, ImageView iv, boolean fixedAspectRatio) {
        if (resultCode == -1) {
            Uri cropUri;
            switch (requestCode) {
                case 256:
                    ...
                    cropImage(act, intentclass, Uri.fromFile(new File(savePath)), cropUri, h.arp, h.arp, fixedAspectRatio);
                    break;
                case 257:
                    if (!(data == null || data.getData() == null)) {
                        cropPath = UtilsFile.getTempFileName();
                        cropUri = Uri.fromFile(new File(cropPath));
                        cropImage(act, intentclass, data.getData(), cropUri, h.arp, h.arp, fixedAspectRatio);
                        break;
                    }
                case 258:
                    ...
                    if (iv != null) {
                        iv.setImageBitmap(UtilsImage.decodeByteArray(UtilsFile.getBytesFromSD(cropPath)));
                    }
                    return cropPath;
                    break;
            }
    ...
    public static void showPhotoMenu(Activity activity) {
        ...
        View layout = LayoutInflater.from(activity).inflate(R.layout.include_dialog_camera, null);
        builder.setCancelable(true);
        AlertDialog dialog = builder.create();
        dialog.setView(layout, 0, 0, 0, 0);
        dialog.show();
        layout.findViewById(R.id.tv_shot).setOnClickListener(new 1(dialog, activity));
        layout.findViewById(R.id.tv_album).setOnClickListener(new 2(dialog, activity));
        layout.findViewById(R.id.tv_cancel).setOnClickListener(new 3(dialog));
    }
}
