...
import com.huluxia.framework.base.widget.stagger.ExtendableListView.FixedViewInfo;
import com.huluxia.framework.base.widget.stagger.ExtendableListView.LayoutParams;
import com.huluxia.framework.base.widget.stagger.ExtendableListView.ListSavedState;
...
public class AnimatedHeaderStaggeredGridView extends ExtendableListView {
    ...
    public AnimatedHeaderStaggeredGridView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public AnimatedHeaderStaggeredGridView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        View head = null;
        if (!(this.mHeaderViewInfos == null || this.mHeaderViewInfos.isEmpty())) {
            head = ((FixedViewInfo) this.mHeaderViewInfos.get(0)).view;
            if (this.startHeight < 0) {
                this.startHeight = head.getMeasuredHeight();
            }
        }
        switch (action) {
            case 0:
                this.downloadX = event.getRawX();
                this.downloadY = event.getRawY();
                this.lastX = this.downloadX;
                this.lastY = this.downloadY;
                HLog.debug(this, "head height %f", new Object[]{Float.valueOf(this.lastY)});
                break;
            case 1:
                if (!isTop()) {
                    this.downloadX = 0.0f;
                ...
                if (this.pullListener != null && head.getLayoutParams().height - this.startHeight > 160) {
                    this.pullListener.onRelase();
                }
                head.getLayoutParams().height = this.startHeight;
                head.requestLayout();
                return true;
            case 2:
                float deltaY;
                ...
                this.lastY = y;
                if (isTop() && (deltaY >= 0.0f || head.getLayoutParams().height > this.startHeight)) {
                    HLog.debug(this, "head height %d, deltaY %f", new Object[]{Integer.valueOf(head.getLayoutParams().height), Float.valueOf(deltaY)});
                    int finalHeight = (int) (((float) head.getLayoutParams().height) + (deltaY / 3.0f));
                    if (finalHeight <= this.startHeight) {
                        finalHeight = this.startHeight;
                    }
                    if (this.pullListener != null) {
                        this.pullListener.onPullDownload(deltaY);
                    }
                    head.getLayoutParams().height = finalHeight;
                    head.requestLayout();
                    return true;
                }
            case 3:
