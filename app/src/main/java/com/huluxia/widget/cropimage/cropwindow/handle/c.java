...
abstract class c {
    ...
    private float getAspectRatio(float x, float y) {
        return com.huluxia.widget.cropimage.util.a.calculateAspectRatio(this.bvZ == Edge.LEFT ? x : Edge.LEFT.getCoordinate(), this.bvY == Edge.TOP ? y : Edge.TOP.getCoordinate(), this.bvZ == Edge.RIGHT ? x : Edge.RIGHT.getCoordinate(), this.bvY == Edge.BOTTOM ? y : Edge.BOTTOM.getCoordinate());
    }
}
