...
public class McReloadFragment extends ReloadFragment {
    protected a aIM;
    private int aIN = 0;
    private CallbackHandler aIO = new 1(this);

    public static McReloadFragment Kl() {
        return a(null);
    }

    public static McReloadFragment a(ReloadStatement statement) {
        Bundle bundle = new Bundle();
        if (statement == null) {
            statement = ReloadStatement.generateDefault();
        }
        bundle.putParcelable("STATEMENT", statement);
        McReloadFragment fragment = new McReloadFragment();
