...
class SpaceCustomStyleFragment$3 extends CallbackHandler {
    ...
    public void onRecvNewSpaceModel(boolean succ, w info, int model, Context context) {
        if (context == this.bhs.getActivity()) {
            ...
            this.bhs.cs(false);
            if (succ) {
                t.l(this.bhs.getActivity(), this.bhs.getResources().getString(m.update_space_succ));
            ...
            t.l(this.bhs.getActivity(), info == null ? this.bhs.getResources().getString(m.update_space_failded) : info.msg);
        }
    }
