...
public class SkinSearchActivity extends HTBaseActivity implements OnClickListener {
    private static final int PAGE_SIZE = 20;
    private static final String TAG = "SkinSearchActivity";
    private c aEV;
    ...
    private SkinDownAdapter bcw;
    private CallbackHandler mCallback = new CallbackHandler(this) {
        final /* synthetic */ SkinSearchActivity bcx;

        {
            this.bcx = this$0;
        }

        @MessageHandler(message = 1540)
        public void onRecvRankingData(boolean succ, f info) {
            HLog.debug(this.bcx.aMn, "onRecvRankingData info = " + info + ", succ = " + succ, new Object[0]);
            HLog.info(SkinSearchActivity.TAG, "搜索成功", new Object[0]);
            this.bcx.aEq.onRefreshComplete();
            this.bcx.aEV.onLoadComplete();
            this.bcx.bbj.removeAllViews();
            if (succ && info != null) {
                if (info.start > 20) {
                    this.bcx.aGH.start = info.start;
                    this.bcx.aGH.more = info.more;
                    this.bcx.aGH.mapList.addAll(info.mapList);
                } else {
                    this.bcx.aGH = info;
                    if (UtilsFunction.empty(this.bcx.aGH.mapList)) {
                        this.bcx.bbj.addView(this.bcx.bbi);
                    }
                }
                this.bcx.bcw.a(this.bcx.aGH.mapList, true);
            } else if (info != null) {
                t.n(this.bcx.aMn, ab.n(info.code, info.msg));
            } else {
                t.n(this.bcx.aMn, "数据请求失败，请下拉刷新重试");
            }
        }

        @MessageHandler(message = 768)
        public void onUnzip(long id) {
            if (this.bcx.bcw != null) {
                this.bcx.bcw.notifyDataSetChanged();
            }
        }
    };
        ...
        this.aIY.addTextChangedListener(new TextWatcher(this) {
            ...
            public void afterTextChanged(Editable s) {
                if (this.aEo.length() > 1) {
                    this.bcx.aJa.setClickable(true);
                    this.bcx.aIZ.setVisibility(0);
                } else if (this.aEo.length() > 0) {
                    this.bcx.aJa.setClickable(true);
                    this.bcx.aIZ.setVisibility(0);
                } else {
                    this.bcx.aJa.setClickable(false);
                    this.bcx.aIZ.setVisibility(4);
                    this.bcx.bcw.HA();
                }
            }
    ...
    protected void Fa() {
        ...
        this.aEV.a(new a(this) {
            ...
            public void onLoadData() {
                if (this.bcx.aGH != null) {
                    this.bcx.f(this.bcx.aGH.start, 20, false);
                }
            }
    ...
    protected void Iy() {
        this.bcw = new SkinDownAdapter(this.aMn);
    }
